import random
from elixirconf.csv import write_csv
...
def generate_product_categories():
    rows = tuple((id, name) for id, name in PRODUCT_CATEGORIES.items())
    write_csv(rows, 'data/product_categories.csv', ("id", "name"))
...
def generate_product_inventory():
    ...
    write_csv(
        ...
        ("id", "product_id", "stock", "supply_delivery_days", "reorder_threshold")
    )
